...
public class StatusJSONConverter {
	...
	static Status createStatus(JSONObject json) throws LibException {
		try {
			...
			if (!json.isNull("thumbnail_picture")) {
				status.setThumbnailPicture(getRawString("thumbnail_picture", json));
			}
			if (!json.isNull("middle_picture")) {
				status.setMiddlePicture(getRawString("middle_picture", json));
			}
			if (!json.isNull("original_picture")) {
				status.setOriginalPicture(getRawString("original_picture", json));
			}
			if (!json.isNull("geo")) {
